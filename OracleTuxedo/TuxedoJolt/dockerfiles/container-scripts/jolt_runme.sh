...
# Author: Judy Liu
#
# Usage: source jolt_runme.sh [TuxedoVersion]
#        TuxedoVersion: 12.1.3 or 12.2.2, 12.2.2 by default
#
cd /u01/oracle/user_projects
rm -rf jolt
cp -r /home/oracle/jolt /u01/oracle/user_projects
cd jolt
if [ ! -z "$1" ]
    then
  if [ "$1" = "12.1.3" ]
...
JSL SRVGRP=GRPJAV SRVID=4
    CLOPT="-A -- -n //localhost:1304 -m1 -M5 -x10 -T1"
...
tmloadcf -y ubbjolt
sed -i "s:@HOSTNAME@:localhost:g" joltclient.java
javac joltclient.java
buildserver -o joltsvr -f joltsvr.c -s TOUPPER
tmloadrepos -i service.rep jrepository
# Boot up the domain
tmboot -y
# Run the client
#./simpcl "If you see this message, jolt ran OK"
java joltclient
# Shutdown the domain
tmshutdown -y
