...
cp ${VERSION}/tuxedotmq${VERSION}.rsp .
cp ${VERSION}/install.sh .
cp ${VERSION}/Dockerfile .

docker build $PROXY_SETTINGS -t oracle/tuxedotmq:${VERSION} .
if [ "$?" = "0" ]
    then
	echo ""
	echo "Tuxedo TMQ Docker image is ready to be used. To create a container, run:"
	echo "docker run -ti -v \${LOCAL_DIR}:/u01/oracle/user_projects oracle/tuxedotmq:${VERSION} /bin/bash"
