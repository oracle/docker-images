...
realm_dn         = os.environ.get('REALM_DN', 'dc=us,dc=oracle,dc=com')
...
while i < len(sys.argv):
    if sys.argv[i] == '-username':
        ...
        odsPassword = sys.argv[i + 1]
        i += 2
    elif sys.argv[i] == '-adminPassword':
        orcladminPassword = sys.argv[i + 1]
        i += 2
...
try:
   ...
   oid_setup(orcladminPassword=orcladminPassword,odsPassword=odsPassword,realmDN=realm_dn)
   oid_instanceStatus(instanceName=instanceName)
   exit()
except:
    print 'Unable to setup OID...'
    exit()
