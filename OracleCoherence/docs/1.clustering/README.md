...
The easiest way to make Coherence work inside Docker containers is to run the containers with the `--net=host` argument. This allows the containers to use their hosts network interfaces rather than using virtualized networks.

##Examples

### 1. Clustering Using Host Networking
When using host networking everything in Coherence will work as normal, the containers will use the host's network stack:
...
1. Running the following command to start a `DefaultCacheServer` on the first machine `coh-demo0`.
...
2. To verify that the container is running properly the Docker logs command can be used:
...
3. A second DefaultCacheServer container can now be started on the other Docker Machine host in exactly the same way except that the container will need to have a different name and host name as these must be unique on the overlay network. The second container can be started with this command:
```
...
4. Again to verify that the container is running properly the Docker logs command can be used:
