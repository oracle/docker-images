...
SHARD_SCRIPT_DIR="${SHARD_SCRIPT_DIR:-/opt/oracle/scripts/sharding}"
...
if [ ${OP_TYPE,,} == "standbyshard" ]; then
   ...
   then
       ...
       if [ -x "${SCRIPT_BASE_DIR}/${START_FILE:-startDB.sh}" ]; then
           "${SCRIPT_BASE_DIR}/${START_FILE:-startDB.sh}";
       else
           "$ORACLE_BASE/${START_FILE:-startDB.sh}";
       fi
   fi
fi   
