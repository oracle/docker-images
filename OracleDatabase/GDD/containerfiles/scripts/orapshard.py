...
class OraPShard:
      """
      This class sets up the primary shard after DB installation.
      """
      _SHARD_NOT_READY_MSG = "Primary shard is still not set up. Exiting..."

      def __init__(self,oralogger,orahandler,oraenv,oracommon):
        """
        Initialize shard setup object for primary DB.
        ...
        try:
          ...
          if hasattr(self, "ocommon") and self.ocommon is not None:
              self.ocommon.log_info_message(str(ex_type.__name__),self.file_name)
              self.ocommon.log_info_message(str(ex_value),self.file_name)
              self.ocommon.log_info_message("\n".join(stack_trace),self.file_name)
          else:
              sys.stderr.write(str(ex_type.__name__) + "\n")
              sys.stderr.write(str(ex_value) + "\n")
              sys.stderr.write("\n".join(stack_trace) + "\n")
      ...
      def _is_shard_ready(self):
          """
          Return shard setup completion status.
          """
          return self.shard_setup_check()

      def _require_shard_ready(self):
          """
          Exit when shard setup is incomplete.
          """
          status = self._is_shard_ready()
          if not status:
             self.ocommon.log_info_message(self._SHARD_NOT_READY_MSG,self.file_name)
             self.ocommon.prog_exit("127")
          return True

      def _shard_lock_files(self):
          """
          Return create/exist lock file paths for liveness handling.
          """
      ...
      def setup(self):
          """
           Set up the shard on primary DB.
          """
          ...
          if self.ocommon.check_key("PRE_STANDBY_SETUP",self.ora_env_dict):
            self._require_shard_ready()
            self.ocommon.pre_standby_setup(self.ora_env_dict)
          elif self.ocommon.check_key("CHECK_LIVENESS",self.ora_env_dict):
             create_db_file_lck, exist_db_file_lck = self._shard_lock_files()
             ...
             if os.path.exists(create_db_file_lck):
                ...
                self.ocommon.log_result_message(False, "Shard liveness check failed: provisioning lock file exists", self.file_name)
                sys.exit(127)
             ...
             status = self._is_shard_ready()
             if not status:
                self.ocommon.log_result_message(False, "Shard liveness check failed: shard setup is incomplete", self.file_name)
                self.ocommon.prog_exit("127")
             self.ocommon.log_info_message("Shard liveness check completed successfully!",self.file_name)
             self.ocommon.log_result_message(True, "Shard liveness check completed successfully", self.file_name)
             sys.exit(0)
          elif self.ocommon.check_key("CHECK_READYNESS",self.ora_env_dict):
             status = self._is_shard_ready()
             if not status:
                self.ocommon.log_info_message("Shard readiness check failed.",self.file_name)
                self.ocommon.log_result_message(False, "Shard readiness check failed: shard setup is incomplete", self.file_name)
                self.ocommon.prog_exit("127")
             self.ocommon.log_info_message("Shard readiness check completed successfully!",self.file_name)
             self.ocommon.log_result_message(True, "Shard readiness check completed successfully", self.file_name)
          elif self.ocommon.check_key("IMPORT_TDE_KEY",self.ora_env_dict):
            status = self._is_shard_ready()
            if not status:
               self.ocommon.log_info_message("Shard does not seem to be ready. Unable to import the TDE key",self.file_name)
               self.ocommon.prog_exit("127")
      ...
      def setup_machine(self):
          """
           Perform compute-level setup before shard operations.
          """
          self.omachine.setup()
      ...
      def reset_passwd(self):
         """
           Reset shard passwords based on deployment mode.
         """
      ...
      #     sqlpluslogincmd='''{0}/bin/sqlplus "/as sysdba"'''.format(self.ora_env_dict["ORACLE_HOME"])
           ...
           self.ocommon.log_info_message("Running reset_shard_setup()",self.file_name)
           ...
           if self.ocommon.check_key("RESET_ENV",self.ora_env_dict):
              if self.ora_env_dict["RESET_ENV"]:
                 if not os.path.isfile(shard_reset_file):
                    ...
                    msg='''Reset env is already completed because {0} exists on this machine; skipping reset.'''.format(shard_reset_file)
                    self.ocommon.log_info_message(msg,self.file_name)                    


      def shard_setup_check(self):
           """
            Check whether shard setup is completed.
           """
#           systemStr='''{0}/bin/sqlplus "/as sysdba"'''.format(self.ora_env_dict["ORACLE_HOME"])
        ...
        #   self.ocommon.set_mask_str(self.ora_env_dict["ORACLE_PWD"])
           ...
           tmp_dir=self.ora_env_dict.get("TMP_DIR","/tmp")
      ...
      def setup_cdb_shard(self):
           """
            Configure shard settings at CDB level.
           """
           self.ocommon.log_info_message("Running setup_cdb_shard()",self.file_name)
           # sqlpluslogincmd='''{0}/bin/sqlplus "/as sysdba"'''.format(self.ora_env_dict["ORACLE_HOME"])            
           # Assigning variable
           sqlpluslogincmd=self._get_sqlplus_sysdba_login()
           ...
           if self._is_racdb():
               ...
               if self.ocommon.check_key("COMMA_SEPARATED_CLS_NODES",self.ora_env_dict):
                  for value in self._require_cluster_nodes():
      ...
      def set_spfile_nonm_params(self):
           ...
           if self.ocommon.check_key("CLONE_DB",self.ora_env_dict):
              if self.ora_env_dict["CLONE_DB"] != 'true':
                  ...
                  msg='''Setting up shard CDB with SPFILE non-modifiable parameters'''
                  self.ocommon.log_info_message(msg,self.file_name)
                  if self._is_racdb():
                     # RAC Database case
                     sqlcmd='''
      ...
      def set_dbparams_version(self):
           ...
           if int(version) > 12:
              ...
              msg='''Setting up shard CDB with version-specific SPFILE non-modifiable parameters'''
              self.ocommon.log_info_message(msg,self.file_name)
...
#                 '''.format(dbf_dest,obase,"dbconfig",dbuname)
      ...
      def setup_pdb_shard(self):
           """
            Configure shard settings at PDB level.
           """
           self.ocommon.log_info_message("Running setup_pdb_shard()",self.file_name)
           sqlpluslogincmd=self._get_sqlplus_sysdba_login()
           # Assigning variable
           self.ocommon.set_mask_str(self.ora_env_dict["ORACLE_PWD"])
...
'''.format(osuser,hostname,inst_sid,self.ora_env_dict["ORACLE_PDB"])
      ...
      def update_shard_setup(self):
           """
            Update shard setup status table.
           """
       #    systemStr='''{0}/bin/sqlplus {1}/{2}'''.format(self.ora_env_dict["ORACLE_HOME"],"system","HIDDEN_STRING")
           self.ocommon.log_info_message("Running update_shard_setup()",self.file_name)
           systemStr=self._get_sqlplus_sysdba_login()
