...
if [ -z "${TDM}" ]; then
   ...
   TDM=`echo "$TDM" | tr '[:upper:]' '[:lower:]'`
   TDM_THREADING_MODE="dedicated"
fi

if [ ! -z "${ENABLE_REST_API}" ]; then
   ENABLE_REST_API=`echo "$ENABLE_REST_API" | tr '[:upper:]' '[:lower:]'`
else 
   ENABLE_REST_API="false"
fi

if [ -z "${REST_HOST}" ]; then
   REST_HOST=`hostname -f`
fi
...
if [ "${ENABLE_REST_API}" == "true" ]; then
   ...
   if [ -z "${WALLET_LOCATION}" ]; then
      export WALLET_LOCATION=$DB_BASE/wallet
   fi
   print_message "REST HOST is defined to $REST_HOST"
   print_message "REST PORT is defined to $REST_PORT"
   REST_PASSWORD=$(openssl pkeyutl -decrypt -in ${REST_PWD_FILE} -inkey ${REST_PWD_KEY})
 else
...
if [ "${ENABLE_REST_API}" == "true" ]; then
   sed -i -e "s|###REST_HOST###|$REST_HOST|g" $logdir/$CMANORA
   sed -i -e "s|###REST_PORT###|$REST_PORT|g" $logdir/$CMANORA
else
   sed -i -e "/(REST_ADDRESS=/d" $logdir/$CMANORA
fi

if [ "${TDM}" == "yes" ]; then
   sed -i -e "s|###TDM###|$TDM|g" $logdir/$CMANORA
   sed -i -e "s|###TDM_THREADING_MODE###|$TDM_THREADING_MODE|g" $logdir/$CMANORA
else
   sed -i -e "/(TDM=/d" $logdir/$CMANORA
   sed -i -e "/(TDM_THREADING_MODE=/d" $logdir/$CMANORA
fi
...
if [ "${ENABLE_REST_API}" == "true" ]; then
   ...
   $ORACLE_HOME/bin/orapki  wallet create -wallet $WALLET_LOCATION -pwd ${REST_PASSWORD}
   ...
   $ORACLE_HOME/bin/orapki wallet add -wallet $WALLET_LOCATION  -dn "CN=$REST_HOST,C=US" -keysize 2048   -self_signed    -validity 365 -pwd ${REST_PASSWORD}

   echo ${REST_PASSWORD} | $ORACLE_HOME/bin/mkstore -wrl $WALLET_LOCATION  -createEntry oracle ${REST_PASSWORD}


   echo "$ORACLE_HOME/bin/orapki wallet create -wallet $WALLET_LOCATION  -auto_login -pwd ${HIDE_PASSWORD}"
   $ORACLE_HOME/bin/orapki wallet create -wallet $WALLET_LOCATION  -auto_login -pwd ${REST_PASSWORD}

   echo "$ORACLE_HOME/bin/orapki wallet export -wallet $WALLET_LOCATION  -dn \"CN=$REST_HOST,C=US\" -cert  $WALLET_LOCATION/RESTAPI.cert"
   $ORACLE_HOME/bin/orapki wallet export -wallet $WALLET_LOCATION  -dn "CN=$REST_HOST,C=US" -cert  $WALLET_LOCATION/RESTAPI.cert
   ...
   $ORACLE_HOME/bin/orapki cert display -cert $WALLET_LOCATION/RESTAPI.cert
   print_message "================================================="
