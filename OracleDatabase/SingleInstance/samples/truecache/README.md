...
Use [configure-primary-truecache-service.sh](configure-primary-truecache-service.sh) as the sample primary-host script for the manual primary-side service step in wallet-backed True Cache deployments. The script creates the primary service if needed, starts it if needed, and then runs the primary-side DBCA True Cache service configuration.
