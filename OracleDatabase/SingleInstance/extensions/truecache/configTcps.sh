...
function upsert_tns_alias_block() {
    ...
    if [ -x "${TNS_ALIAS_HELPER}" ]; then
        # Client wallet aliases for TCPS must use PROTOCOL=TCPS (helper defaults to TCP).
        local args=(--file "$tns_file" --alias "$alias_name" --host "$host" --port "$port" --service "$service_name" --protocol TCPS --upsert)
        if [ -n "$ssl_dn" ]; then
