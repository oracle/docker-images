...
# dbcaUtils.sh must be packaged next to this script (see Dockerfile COPY).
if [ ! -f "${SCRIPT_DIR}/dbcaUtils.sh" ]; then
   echo "ERROR: ${SCRIPT_DIR}/dbcaUtils.sh not found."
   echo "The True Cache extension image must include dbcaUtils.sh with createService.sh."
   echo "Rebuild with extensions/buildExtensions.sh -x truecache ..."
   exit 1
fi
