#!/bin/bash
# shellcheck disable=SC2034,SC2086,SC2153,SC2155,SC2181,SC2236
...
# Check whether block change tracking is currently enabled.
# Return tri-state status so restore logic can react safely.
function isBCTEnabled() {
...
# Restore and recover the database to ORADATA_DIR, then open RESETLOGS.
# Disable block change tracking first when the source database has it enabled.
