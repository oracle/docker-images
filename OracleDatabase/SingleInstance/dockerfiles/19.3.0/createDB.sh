...
function ensureStandbyDuplicateStaticListener {
  local listener_file="${ORACLE_HOME}/network/admin/listener.ora"
  local status_file="/tmp/listener_status_${ORACLE_SID}"
  local retries=600
  ...
  while (( retries > 0 )); do
    if [[ -f "${listener_file}" ]]; then
      if ! grep -q "SID_NAME[[:space:]]*=[[:space:]]*${ORACLE_SID}" "${listener_file}"; then
        cat >> "${listener_file}" <<EOF

# Static listener entry required for RMAN duplicate auxiliary connection.
SID_LIST_LISTENER =
  (SID_LIST =
    (SID_DESC =
...
EOF
      ...
      if lsnrctl status > "${status_file}" 2>&1 &&
         grep -q "Service \"${ORACLE_SID}\"" "${status_file}" &&
         grep -q "status UNKNOWN" "${status_file}"; then
        echo "Static listener entry active for standby duplicate: ${ORACLE_SID}"
        return 0
      fi
    fi
  ...
  echo "WARNING: Static listener entry was not confirmed for standby duplicate: ${ORACLE_SID}"
  return 0
}
...
function prepareStandbyTDEWalletFromZip {
  ...
  if ! find "${TDE_WALLET_ROOT}" -maxdepth 3 -type f \
       \( -name "cwallet.sso" -o -name "ewallet.p12" \) |
       grep -q .; then
    echo "ERROR: No wallet files found under ${TDE_WALLET_ROOT}. Exiting..."
    exit 1
  fi

  find "${TDE_WALLET_ROOT}" -type d -exec chmod 700 {} \; || true
  find "${TDE_WALLET_ROOT}" -type f -exec chmod 600 {} \; || true

  export TDE_WALLET_ROOT
  echo "Standby TDE wallet prepared under ${TDE_WALLET_ROOT}"
}
...
if [[ "${CLONE_DB}" == "true" ]] || [[ "${STANDBY_DB}" == "true" ]]; then
  ...
  if [ "${STANDBY_DB}" = "true" ]; then
    ...
    STANDBY_STATIC_LISTENER_PID=""
    STANDBY_TDE_PATCH_PID=""

    if [[ "${TDE_ENABLED}" == "true" ]]; then
      prepareStandbyTDEWalletFromZip

      ensureStandbyDuplicateStaticListener &
      STANDBY_STATIC_LISTENER_PID=$!

      # DBCA imports tde_configuration from the primary.
      # Pass only wallet_root here.
      TDE_DBCA_INIT_PARAMS=(
        -initParams
    ...
    if [[ "${TDE_ENABLED}" == "true" ]]; then
      # TDE standby only: DBCA must run in the background so its generated
      # PFILE can be corrected before each DBCA startup operation.
      ...
      if [[ "${DBCA_RC}" -ne 0 ]]; then
        if [[ -n "${STANDBY_STATIC_LISTENER_PID}" ]]; then
          wait "${STANDBY_STATIC_LISTENER_PID}" || true
        fi
      ...
      # Existing non-TDE standby behavior remains unchanged.
      # shellcheck disable=SC2086
