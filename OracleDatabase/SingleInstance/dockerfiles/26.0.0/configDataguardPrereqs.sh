...
function broker_config_files_are_valid() {
  local db_unique_name file1 file2
  db_unique_name="$("$SQLPLUS_BIN" -s / as sysdba <<'EOF' 2>/dev/null
set pages 0 head off feedback off verify off echo off
select trim(value) from v$parameter where name = 'db_unique_name';
exit
EOF
)"
  db_unique_name="$(printf '%s' "$db_unique_name" | awk 'NF { value=$0 } END { print value }')"
  if [ -z "$db_unique_name" ]; then
    echo "BROKER_CONFIG_FILES_VALID=false"
    return 0
  fi
  file1="$DG_BROKER_CONFIG_DIR/dr1$db_unique_name.dat"
  file2="$DG_BROKER_CONFIG_DIR/dr2$db_unique_name.dat"
  if [ -s "$file1" ] && [ -s "$file2" ]; then
    echo "BROKER_CONFIG_FILES_VALID=true"
  else
    echo "BROKER_CONFIG_FILES_VALID=false"
  fi
  echo "BROKER_CONFIG_FILE1=$file1"
  echo "BROKER_CONFIG_FILE2=$file2"
}
...
begin
  ...
  if l_broker_enabled = 'true' then
    ...
    declare
      l_current_file1 varchar2(4000);
      l_current_file2 varchar2(4000);
      l_current_start varchar2(20);
    begin
      select max(case when name = 'dg_broker_config_file1' then value end),
             max(case when name = 'dg_broker_config_file2' then value end),
             max(case when name = 'dg_broker_start' then upper(value) end)
        into l_current_file1, l_current_file2, l_current_start
        from v\$parameter
       where name in ('dg_broker_config_file1', 'dg_broker_config_file2', 'dg_broker_start');
      if l_current_file1 = l_file1 and l_current_file2 = l_file2 and l_current_start = 'TRUE' and '$DG_BROKER_CONFIG_FILES_VALID' = 'true' then
        dbms_output.put_line('INFO: broker parameters and files already configured; leaving DMON running');
      else
        if l_current_file1 = l_file1 and l_current_file2 = l_file2 and '$DG_BROKER_CONFIG_FILES_VALID' != 'true' then
          raise_application_error(-20081, 'broker configuration files are missing or empty; refusing to restart DMON');
        end if;
        if l_current_file1 != l_file1 or l_current_file2 != l_file2 then
...
if [ "${ACTION}" = "status" ]; then
  ...
  export DG_BROKER_CONFIG_FILES_VALID=false
  if [ "$DG_ENABLE_BROKER" = "true" ]; then
    broker_config_status="$(broker_config_files_are_valid)"
    printf '%s\n' "$broker_config_status"
    if printf '%s\n' "$broker_config_status" | grep -q '^BROKER_CONFIG_FILES_VALID=true$'; then
      export DG_BROKER_CONFIG_FILES_VALID=true
    fi
  fi
  configure_local_dg_net
  run_configure_sql
