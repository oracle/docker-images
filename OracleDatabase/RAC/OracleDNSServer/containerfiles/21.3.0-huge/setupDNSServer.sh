...
setEnvVariables()
{
    ...
    RAC_PUBLIC1_SUBNET="192.168.17"
    export RAC_PUBLIC1_SUBNET
    print_message "RAC_PUBLIC1_SUBNET is set to $RAC_PUBLIC1_SUBNET"
    PUB_IP_DIGIT_3=`echo $RAC_PUBLIC1_SUBNET | cut -d"." -f3`
    PUB_IP_DIGIT_2=`echo $RAC_PUBLIC1_SUBNET | cut -d"." -f2`
    PUB_IP_DIGIT_1=`echo $RAC_PUBLIC1_SUBNET | cut -d"." -f1`
    RAC_PUBLIC_REVERSE1_IP="${PUB_IP_DIGIT_3}.${PUB_IP_DIGIT_2}.${PUB_IP_DIGIT_1}"
    print_message "RAC_PUBLIC_REVERSE1_IP set to $RAC_PUBLIC_REVERSE1_IP"

    for i in {1..8}
    do
      export RAC_PUBLIC${i}_SUBNET="${PUB_IP_DIGIT_1}.${PUB_IP_DIGIT_2}.${PUB_IP_DIGIT_3}"
    ...
    RAC_PRIVATE_SUBNET="192.168.100"
    export RAC_PRIVATE1_SUBNET=$RAC_PRIVATE_SUBNET
    PRIV_IP_DIGIT_3=`echo $RAC_PRIVATE1_SUBNET | cut -d"." -f3`
    PRIV_IP_DIGIT_2=`echo $RAC_PRIVATE1_SUBNET | cut -d"." -f2`
    PRIV_IP_DIGIT_1=`echo $RAC_PRIVATE1_SUBNET | cut -d"." -f1`
    RAC_PRIVATE1_REVERSE1_IP="${PRIV_IP_DIGIT_3}.${PRIV_IP_DIGIT_2}.${PRIV_IP_DIGIT_1}"
    print_message "RAC_PRIVATE1_REVERSE1_IP set to $RAC_PRIVATE1_REVERSE1_IP"

    for i in {1..8}
    do
      export RAC_PRIVATE${i}_SUBNET="${PRIV_IP_DIGIT_1}.${PRIV_IP_DIGIT_2}.${PRIV_IP_DIGIT_3}"
      export RAC_PRIVATE1_REVERSE${i}_IP="${PRIV_IP_DIGIT_3}.${PRIV_IP_DIGIT_2}.${PRIV_IP_DIGIT_1}"
      export TMPSTR=\$RAC_PRIVATE1_REVERSE${i}_IP
      PRIV_IP_DIGIT_3=`expr $PRIV_IP_DIGIT_3 + 1`
    done

    RAC_PRIVATE1_SUBNET2="192.168.200"
    export RAC_PRIVATE1_SUBNET2
    print_message "RAC_PRIVATE1_SUBNET2 is set to $RAC_PRIVATE1_SUBNET2"
    
    PRIV2_IP_DIGIT_3=`echo $RAC_PRIVATE1_SUBNET2 | cut -d"." -f3`
    PRIV2_IP_DIGIT_2=`echo $RAC_PRIVATE1_SUBNET2 | cut -d"." -f2`
    PRIV2_IP_DIGIT_1=`echo $RAC_PRIVATE1_SUBNET2 | cut -d"." -f1`
    RAC_PRIVATE2_REVERSE1_IP="${PRIV2_IP_DIGIT_3}.${PRIV2_IP_DIGIT_2}.${PRIV2_IP_DIGIT_1}"
    print_message "RAC_PRIVATE2_REVERSE1_IP set to $RAC_PRIVATE2_REVERSE1_IP"

    for i in {1..8}
    do
      export RAC_PRIVATE${i}_SUBNET2="${PRIV2_IP_DIGIT_1}.${PRIV2_IP_DIGIT_2}.${PRIV2_IP_DIGIT_3}"
      export RAC_PRIVATE2_REVERSE${i}_IP="${PRIV2_IP_DIGIT_3}.${PRIV2_IP_DIGIT_2}.${PRIV2_IP_DIGIT_1}"
      export TMPSTR=\$RAC_PRIVATE2_REVERSE${i}_IP
      PRIV2_IP_DIGIT_3=`expr $PRIV2_IP_DIGIT_3 + 1`
    done

    RAC_PUBLIC1_VIP_SUBNET="192.168.25"
    export RAC_PUBLIC1_VIP_SUBNET
    print_message "RAC_PUBLIC1_VIP_SUBNET is set to $RAC_PUBLIC1_VIP_SUBNET"

    VIP_IP_DIGIT_3=`echo $RAC_PUBLIC1_VIP_SUBNET | cut -d"." -f3`
    VIP_IP_DIGIT_2=`echo $RAC_PUBLIC1_VIP_SUBNET | cut -d"." -f2`
    VIP_IP_DIGIT_1=`echo $RAC_PUBLIC1_VIP_SUBNET | cut -d"." -f1`
    PUBLIC_VIP_REVERSE1_IP="${VIP_IP_DIGIT_3}.${VIP_IP_DIGIT_2}.${VIP_IP_DIGIT_1}"
    print_message "PUBLIC_VIP_REVERSE1_IP set to $PUBLIC_VIP_REVERSE1_IP"

    for i in {1..8}
    do
      export RAC_PUBLIC${i}_VIP_SUBNET="${VIP_IP_DIGIT_1}.${VIP_IP_DIGIT_2}.${VIP_IP_DIGIT_3}"
      export PUBLIC_VIP_REVERSE${i}_IP="${VIP_IP_DIGIT_3}.${VIP_IP_DIGIT_2}.${VIP_IP_DIGIT_1}"
      export TMPSTR=\$PUBLIC_VIP_REVERSE${i}_IP
      VIP_IP_DIGIT_3=`expr $VIP_IP_DIGIT_3 + 1`
    done

    RAC_PUBLIC1_SVIP_SUBNET="192.168.33"
    export RAC_PUBLIC1_SVIP_SUBNET
    print_message "RAC_PUBLIC1_SVIP_SUBNET is set to $RAC_PUBLIC1_SVIP_SUBNET"

    SVIP_IP_DIGIT_3=`echo $RAC_PUBLIC1_SVIP_SUBNET | cut -d"." -f3`
    SVIP_IP_DIGIT_2=`echo $RAC_PUBLIC1_SVIP_SUBNET | cut -d"." -f2`
    SVIP_IP_DIGIT_1=`echo $RAC_PUBLIC1_SVIP_SUBNET | cut -d"." -f1`
    PUBLIC_SVIP_REVERSE1_IP="${SVIP_IP_DIGIT_3}.${SVIP_IP_DIGIT_2}.${SVIP_IP_DIGIT_1}"
    print_message "PUBLIC_SVIP_REVERSE1_IP set to $PUBLIC_SVIP_REVERSE1_IP"

    for i in {1..8}
    do
      export RAC_PUBLIC${i}_SVIP_SUBNET="${SVIP_IP_DIGIT_1}.${SVIP_IP_DIGIT_2}.${SVIP_IP_DIGIT_3}"
      export PUBLIC_SVIP_REVERSE${i}_IP="${SVIP_IP_DIGIT_3}.${SVIP_IP_DIGIT_2}.${SVIP_IP_DIGIT_1}"
      export TMPSTR=\$PUBLIC_SVIP_REVERSE${i}_IP
      SVIP_IP_DIGIT_3=`expr $SVIP_IP_DIGIT_3 + 1`
    done
...
setupZoneFile ()
{
   ...
   sed -i -e "s|###HOSTNAME_IP_LAST_DIGITS###|$HOSTNAME_IP_LAST_DIGITS|g" $ZONE_FILE_LOC_2/${DOMAIN_NAME}.zone
    for i in {1..8}
    do
      TMPSTR=\$RAC_PUBLIC${i}_VIP_SUBNET
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###RAC_PUBLIC${i}_VIP_SUBNET###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${DOMAIN_NAME}.zone

      TMPSTR=\$RAC_PUBLIC${i}_SVIP_SUBNET
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###RAC_PUBLIC${i}_SVIP_SUBNET###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${DOMAIN_NAME}.zone

      TMPSTR=\$RAC_PUBLIC${i}_SUBNET
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###RAC_PUBLIC${i}_SUBNET###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${DOMAIN_NAME}.zone
    done
}
...
setupReverseZonefile()
{
   ...
   sed -i -e "s|###HOSTNAME_IP_LAST_DIGITS###|$HOSTNAME_IP_LAST_DIGITS|g" $ZONE_FILE_LOC_2/${DOMAIN_NAME}.rzone
    for i in {1..8}
    do
      TMPSTR=\$PUBLIC_VIP_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###PUBLIC_VIP_REVERSE${i}_IP###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${DOMAIN_NAME}.rzone

      TMPSTR=\$PUBLIC_SVIP_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###PUBLIC_SVIP_REVERSE${i}_IP###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${DOMAIN_NAME}.rzone

      TMPSTR=\$RAC_PUBLIC_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###RAC_PUBLIC_REVERSE${i}_IP###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${DOMAIN_NAME}.rzone
    done
...
setupPrivateZoneFile ()
{
   ...
   for i in {1..8}
   do
      TMPSTR=\$RAC_PRIVATE${i}_SUBNET
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###RAC_PRIVATE${i}_SUBNET###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${PRIVATE_DOMAIN_NAME}.zone

      TMPSTR=\$RAC_PRIVATE${i}_SUBNET2
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###RAC_PRIVATE${i}_SUBNET2###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${PRIVATE_DOMAIN_NAME}.zone
   done
...
setupPrivateReverseZonefile()
{
   ...
   for i in {1..8}
   do
      TMPSTR=\$RAC_PRIVATE1_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###RAC_PRIVATE1_REVERSE${i}_IP###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${PRIVATE_DOMAIN_NAME}.rzone

      TMPSTR=\$RAC_PRIVATE2_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###RAC_PRIVATE2_REVERSE${i}_IP###|$TMPREVIP|g" $ZONE_FILE_LOC_2/${PRIVATE_DOMAIN_NAME}.rzone
  done
}
...
setupNamed()
{
   ...
   do
      TMPSTR=\$RAC_PUBLIC_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###REVERSE${i}_IP###|$TMPREVIP|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE

      TMPSTR=\$PUBLIC_VIP_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###PUBLIC_VIP_REVERSE${i}_IP###|$TMPREVIP|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE

      TMPSTR=\$PUBLIC_SVIP_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###PUBLIC_SVIP_REVERSE${i}_IP###|$TMPREVIP|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE

      TMPSTR=\$RAC_PRIVATE1_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###PRIVATE1_REVERSE${i}_IP###|$TMPREVIP|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE

      TMPSTR=\$RAC_PRIVATE2_REVERSE${i}_IP
      TMPREVIP=`eval echo ${TMPSTR}`
      sed -i -e "s|###PRIVATE2_REVERSE${i}_IP###|$TMPREVIP|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE
   done
}
