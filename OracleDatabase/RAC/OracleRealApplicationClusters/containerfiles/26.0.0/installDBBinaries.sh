...
if [ "$DB_EDITION" != "EE" -a "$DB_EDITION" != "SE2" ]; then
...
# ---------------------
#sed -i -e "s|###ORACLE_EDITION###|$DB_EDITION|g" "$INSTALL_SCRIPTS"/"$DB_INSTALL_RSP" && \
...
"$DB_HOME"/runInstaller -silent -setupDBHome -OSDBA dba -OSOPER oper -OSBACKUPDBA backupdba -OSDGDBA dgdba -OSKMDBA kmdba -OSRACDBA racdba -ORACLE_BASE ${DB_BASE} -installEdition EE -INVENTORY_LOCATION ${INVENTORY} -ignorePrereqFailure || true
