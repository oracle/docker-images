...
######### Add RAC DB ########
    ...
    def _resolve_scan_ipv4_addresses(self):
        """
        Resolve all unique IPv4 addresses currently published for SCAN_NAME.
        """
        scanname = self.ora_env_dict["SCAN_NAME"]
        try:
            address_info = socket.getaddrinfo(scanname, None, socket.AF_INET)
        except socket.gaierror as error:
            self.log_warn_message(
                "Unable to resolve SCAN name {0}: {1}".format(scanname, error),
                self.file_name,
            )
            return []

        return sorted(set([entry[4][0] for entry in address_info]))

    def _get_remote_cluster_node_states(self, user, home, node):
        """
        Return cluster node states from remote olsnodes -s output.
        """
        path, ldpath = self._oracle_env_paths(home)
        cmd = '''su - {0} -c "ssh {1} 'export ORACLE_HOME={2};export PATH={3};export LD_LIBRARY_PATH={4};{2}/bin/olsnodes -s'"'''.format(
...
######### Add RAC DB ########
    ...
    def reconcile_scan_resources(self, user, home, node, expected_count=None, max_attempts=40, sleep_seconds=15):
        """
        ...
        scan_output = "SCAN status not queried because cluster readiness did not converge."
        lsnr_output = "SCAN listener status not queried because cluster readiness did not converge."
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            expected_nodes = self._get_ordered_scan_nodes(node)[:expected_count]
            node_states = self._get_remote_cluster_node_states(user, home, node)
            active_nodes = sorted([
                cluster_node for cluster_node, state in node_states.items()
                if state.lower() == "active"
            ], key=self._node_sort_key)
            scan_addresses = self._resolve_scan_ipv4_addresses()
            nodes_ready = (
                len(expected_nodes) == expected_count and
                all(node_states.get(expected_node, "").lower() == "active"
                    for expected_node in expected_nodes)
            )
            addresses_ready = len(scan_addresses) >= expected_count

            self.log_info_message(
                "SCAN readiness: expected_nodes={0}, active_nodes={1}, addresses={2}, expected_count={3}".format(
                    expected_nodes, active_nodes, scan_addresses, expected_count
                ),
                self.file_name,
            )

            if not nodes_ready or not addresses_ready:
