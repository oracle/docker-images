...
yum -y install systemd oracle-database-preinstall-19c net-tools which zip unzip tar openssl expect e2fsprogs openssh-server openssh-client gcc-c++ vim-minimal passwd which sudo && \
yum clean all 
