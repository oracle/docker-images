...
```
**Version folders for automation (dbInstall mapping):**

| Folder | Use for |
|--------|---------|
| `19.3.0` | 19c (folder name kept; zip build-args for later RUs) |
| `21.3.0` | 21c |
| `23.26.0` | All 23.26.* builds — default media `LINUX.X64_2326100_grid_home.zip` + `LINUX.X64_2326100_db_home.zip` |
| `26.0.0` | MAIN / 26ai |
...
Example: 23.26 track with default 2326100 media:
```bash
...
```
Example: Slim image for 23.26:
```bash
./buildContainerImage.sh -v 23.26.0 -i -o '--build-arg SLIMMING=true'
```
Example: Slim image for MAIN / 26ai:
```bash
...
```
Example: Base image for MAIN / 26ai:
```bash
./buildContainerImage.sh -v 26.0.0 -b
```
Example: Base image for 23.26 track:
```bash
./buildContainerImage.sh \
    -v 23.26.0 \
    -b \
    -t localhost/oracle/database-rac:23.26.0-base \
    -o "--build-arg INSTALL_FILE_1=LINUX.X64_2326100_grid_home.zip --build-arg INSTALL_FILE_2=LINUX.X64_2326100_db_home.zip"
```
