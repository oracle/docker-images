...
```
**Primary / current version folders for automation (dbInstall mapping):**
...
- Usage of `./buildContainerImage.sh`:
   ...
   ```
- After Oracle RAC Container image is built, you can also apply RU and one-off patches to build the patched image. For example, after building a base image from folder `19.3.0` or `23.26.0`, refer to [Example of how to create an Oracle RAC Database Container Patched Image](./samples/applypatch/README.md).
