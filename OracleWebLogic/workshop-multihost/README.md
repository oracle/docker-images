...
To give users an idea on how to create a WebLogic Server Domain in a Multi Host environment we have scripts under '~/docker-images/OracleWebLogic/samples/1221-multihost/1221-multihost' directory.  The **bootstrap.sh** script starts 2 Docker Machines the **weblogic-orchestrator** and **weblogic-master** . The **weblogic-orchestrator** Docker Machine has the **Docker Registry** running, where we register the images we need to run containers from, and the **Consul** to help us start services. The **weblogic-master** Docker Machine has the **Docker Swarm, the Overlay Network, and the WebLogic Admin Server container** running in the VM. The **1221-appdeploy** image is pushed into the registry running in the Orchestrator machine.  Finally the bootstrap script calls post-bootstrap script to start an Admin Server container on the **weblogic-master** machine.
...
![](images/17_console_deployments.png)
